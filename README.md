...
To be able to follow along this article, you need to have experience issuing commands through a terminal. You should also have some knowledge of Git version control and PHP specifically with the Laravel framework.
...
Cloudinary is great platform at media management and manipulation. It is perfect for our current use case since we will need to apply some transformations to the images our users will be uploading and return a URL which we will use to populate our gallery.

To return their desired photo booth effect we will manipulate the photos as follows:

1. Change the aspect ratio 0.75 that is a 3x4.
...
As you can see we will have four overlays in this project. You can download them above and upload them to the Cloudinary Media library and name them as follows:
...
- ### Overlay Transformation
  ...
  First, we create the effect transformations based on user input and assign it to the variable `$effectTransformations`.
